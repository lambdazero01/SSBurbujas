...
if times:
    ...
    with open(filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['time_s', 'raw_flujo', 'flow_crudo_slm', 'flow_filtrado_slm', 'volume_total_L', 'burb_count', 'new_bubbles'])
        for i in range(len(times)):
            writer.writerow([times[i], raw_flujos[i], flows[i], flows_filtered[i], volumes[i], burb_counts[i], new_bubbles_list[i]])
