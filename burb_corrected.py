...
def obtener_clima(lat, lon):
    ...
    lat/lon: Coordenadas GPS para Puebla.
...
lat, lon = 19.0036, -97.8883  # NEW: Hardcodeadas para Puebla
        ...
        if line:
            ...
            if len(parts) == 3:
                try:
                    ...
                    factor = ((temp_actual + 273.15) / T_std) * (P_std / press_actual)

                    # Integrar si > umbral de ruido
                    if abs(flujo_filtered) > noise_threshold:
                        volume_total += flujo_filtered * (dt / 60.0) * factor
                    ...
                    print(f"Flujo crudo: {flujo:.4f} slm, Flujo filtrado: {flujo_filtered:.4f} slm, Burbujas: {burb_count}, Volumen Total: {volume_total:.4f} L, Volumen por Burbuja: {vol_por_burbuja:.4f} L, Temp: {temp_actual}°C, Press: {press_actual:.1f} hPa, Factor corrección: {factor:.2f}")
                except ValueError:
...
if times:
    ...
    if bubble_vols := [volume_total / burb_count if burb_count > 0 else 0] * len(bubble_times):
...
if times:
    ...
    if bubble_times:
        ...
        with open(bubble_filename, 'w', newline='') as f_bub:
            ...
            vol_avg_bubble = volume_total / burb_count if burb_count > 0 else 0.0
